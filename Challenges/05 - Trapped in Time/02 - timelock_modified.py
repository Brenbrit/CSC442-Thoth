...
def get_server_time():
    ...
    server_time = s.recv(8192).decode().rstrip()[0:24]
...
def calculate_code(epoch_time):
    ...
    if "TIME_SERVER" in globals():
        ...
        current_time = dt.datetime.strptime(server_time, "%a %d %b %Y %H:%M:%S")
...
if "DEBUG_EPOCH_TIME" in globals():
    epoch = dt.datetime.strptime(DEBUG_EPOCH_TIME, TIME_FORMAT)
...
for line in sys.stdin:
    ...
    epoch = dt.datetime.strptime(line_stripped, TIME_FORMAT)
